<div class="m-4">
                ...
                <x-select wire:model='classroom_id'>
                    @slot('content')
                        ...
                        @foreach ($classrooms as $classroom)
                            <option value="{{ $classroom->id }}">{{ $classroom->name }} - {{ $classroom->area->name }}</option>
                        @endforeach
                    @endslot
                </x-select>
    ...
    <x-template-list>
                ...
                @slot('body')
                    @foreach ($assists as $attendance)
                        <tr
                            ...
                            <td class="px-6 py-4">
                                {{ $attendance->classroom->name }} - {{ $attendance->classroom->area->name }}
                            </td>

                            <td class="px-6 py-4 whitespace-nowrap text-left">
                                <ul>
                                    <li>
                                        <a wire:click='edit({{ $attendance->id }})'
                                    ...
                                    <li>
                                        <a wire:click='modalDelete({{ $attendance->id }})'
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-left">
                                <ul>
                                    <li>
                                        <a href="{{ route('page.listing', $attendance) }}"
                                            class="font-medium text-blue-600 dark:text-blue-500 hover:underline cursor-pointer">
                                            {{ __('Listing') }}
                                        </a>
                                    </li>
        ...
        @slot('paginate')
            {{ $assists->links('vendor.livewire.custom') }}
        @endslot
    </x-template-list>
